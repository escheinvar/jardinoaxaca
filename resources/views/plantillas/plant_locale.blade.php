<select wire:change="idiomas()" wire:model="idioma" id="MiIdioma" class="form-control lenguas" style="width:90px; display:inline-block;">
    <option value="es" {{ session('locale') == 'es' ? 'selected' : '' }}>Español</option>
    @if(isset($lenguas))
        @if(in_array('es_mix', $lenguas))
            <option value="es_mix" {{ session('locale') == 'es_mix' ? 'selected' : '' }}>Mixteco</option>
        @endif
